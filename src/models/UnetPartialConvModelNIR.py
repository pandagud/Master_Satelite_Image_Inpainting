...
class generatorNIR(nn.Module):
    def __init__(self):
        super(generatorNIR, self).__init__()

        self.down1 = UNetDown(4, 64, 3, normalize=False)  # self.down(input)??
        ...
        self.up8 = UNetUp(64+4, 4, 3, lastLayer=True) #check opløsning der kommer ud?
...
class criticWgan_NIR(nn.Module):

    # discriminator model
    def __init__(self):
        super(criticWgan_NIR, self).__init__()
