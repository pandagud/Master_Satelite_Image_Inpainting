...
class BaseProcessor(object):
    ...
    def unzip_product(self, product, use_tile_path=False):
        if not product['product_path'].values[0].exists() or self.overwrite_products:
            ...
            logging.debug((str(self.directory)))
            logging.debug(str((self.directory / 'zipfiles' / file_name).with_suffix('.zip')))
            try:
                ...
                logging.info('BadZipfile for product ' + product['title'].values[0])
    ...
    def reproject_sat1_image(self,src, dst, crs='EPSG:3857'):
        src_tmp = None
        ...
        warp_options = gdal.WarpOptions(dstSRS=crs, resampleAlg='cubic', multithread=True,
                                        creationOptions=creation_options,xRes=10,yRes=10) 
    ...
    def reproject_image(self, src, dst, crs='EPSG:3857'):
        src_tmp = None
        if src == dst:  # If you want to replace the src file with the output of this function
            src_tmp = Path(src.parent, 'temp' + src.suffix)  # Create path for temp file
            src.rename(src_tmp)  # Rename the file to the temp filename
