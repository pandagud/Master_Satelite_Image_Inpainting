...
def define_flags():
    ...
    flags.DEFINE_string('end_data_directory','/data/','Sat paper specific')
    ...
    # Sentinel-1
    flags.DEFINE_multi_integer('s1_relative_orbit', [0], 'Relative orbit number (0 => all relative orbits)') ## was 44
    flags.DEFINE_bool('s2_intersection',True,'Only relevant for combing s1 based on s2 input')
    flags.DEFINE_string('s2_order_id','Empty','Only relevant if s2_intersection is true')
    # Sentinel-2
    flags.DEFINE_multi_integer('s2_relative_orbit', [0], 'Relative orbit number (0 => all relative orbits)')
