...
def main(args):
    ...
    if config.model_name == 'PartialConvolutions':
        curtraingModel=trainInpainting_test(train,test,generator,discriminator,config)
        local_model_path=curtraingModel.trainGAN()
    elif config.model_name == 'PartialConvolutionsWgan':
        curtraingModel = trainInpaintingWgan(train, test, Wgangenerator, criticWgan, config)
        local_model_path=curtraingModel.trainGAN()
    ...
    #local_model_path= Path(r"/outputs/JacobPedersen/Satellite_Image_Inpainting_shared/experiments/5575/models/PartialConvolutionsWgan_200.pt")
    #local_model_path=Path(r"C:\Users\panda\PycharmProjects\Image_Inpainting_Sat\Master_Satelite_Image_Inpainting\OutputModels\PartialConvolutionsWgan_200pt")
